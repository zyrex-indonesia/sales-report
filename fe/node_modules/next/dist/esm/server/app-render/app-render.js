...
async function spawnDynamicValidationInDev(resolveValidation, tree, ctx, isNotFound, clientReferenceManifest, route) {
    ...
    let reactServerStream = await workUnitAsyncStorage.run(firstAttemptServerPrerenderStore, scheduleInSequentialTasks, ()=>{
    ...
    try {
         ...
         */ const PRERENDER_COMPLETE = 'NEXT_PRERENDER_COMPLETE';
                ...
                firstAttemptRSCPayload, clientReferenceManifest.clientModules, {
                    ...
                    if ((process.env.NEXT_DEBUG_BUILD || process.env.__NEXT_VERBOSE_LOGGING) && !prospectiveRenderFlightController.signal.aborted && !isPrerenderInterruptedError(err) && !isDynamicServerError(err)) {
                        printDebugThrownValueForProspectiveRender(err, workStore.route);
                    }
                });
                ...
                let clientDynamicTracking = createDynamicTrackingState(renderOpts.isDebugDynamicAccesses);
                ...
                let dynamicValidation = createDynamicValidationState();
                function SSROnError(err, errorInfo) {
                    if (isPrerenderInterruptedError(err) || SSRController.signal.aborted) {
                        SSRIsDynamic = true;
                        const componentStack = errorInfo.componentStack;
                ...
                const prerender = require('react-dom/static.edge').prerender;
                let { prelude, postponed } = await prerenderAndAbortInSequentialTasks(()=>workUnitAsyncStorage.run(ssrPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                ...
                if (clientDynamicTracking.syncDynamicErrorWithStack) {
                    ...
                    // Reset tracking objects
                    clientDynamicTracking = createDynamicTrackingState(renderOpts.isDebugDynamicAccesses);
                    dynamicValidation = createDynamicValidationState();
                    SSRIsDynamic = false;
                    ...
                    ({ prelude, postponed } = await prerenderAndAbortInSequentialTasks(()=>workUnitAsyncStorage.run(finalSSRPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                            reactServerStream: reactServerResult.asUnclosingStream(),
                            preinitScripts: preinitScripts,
                            clientReferenceManifest: clientReferenceManifest,
                            ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
         ...
         */ const cache = workStore.incrementalCache;
                ...
                try {
                    ...
                    if (process.env.NEXT_DEBUG_BUILD && err !== abortReason && !isPrerenderInterruptedError(err) && !isDynamicServerError(err)) {
                        printDebugThrownValueForProspectiveRender(err, workStore.route);
                    }
                }
                ...
                const finalRenderFlightController = new AbortController();
                const finalRenderFlightSignal = finalRenderFlightController.signal;
                let serverDynamicTracking = createDynamicTrackingState(renderOpts.isDebugDynamicAccesses);
                reactServerIsDynamic = false;
                let SSRIsDynamic = false;
                const finalRenderPrerenderStore = prerenderStore = {
                    type: 'prerender',
                ...
                let htmlStream;
                try {
                    htmlStream = await prerenderAndAbortInSequentialTasks(async ()=>{
                        ...
                        const prerender = require('react-dom/static.edge').prerender;
                        const { prelude } = await workUnitAsyncStorage.run(ssrPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                ...
                if (clientDynamicTracking.syncDynamicErrorWithStack) {
                    ...
                    SSRIsDynamic = false;
                    reactServerIsDynamic = false;
                    serverDynamicTracking = createDynamicTrackingState(renderOpts.isDebugDynamicAccesses);
                    ...
                    const thirdRenderRSCPayload = await workUnitAsyncStorage.run(finalRenderPrerenderStore, getRSCPayload, tree, ctx, res.statusCode === 404);
                    const finalSSRController = new AbortController();
                    const finalSSRPrerenderStore = {
                        type: 'prerender',
                        phase: 'render',
                    ...
                    try {
                        htmlStream = await prerenderAndAbortInSequentialTasks(()=>{
                            ...
                            const rscForSSRStream = teedStream[1];
                            const renderToReadableStream = require('react-dom/server.edge').renderToReadableStream;
                            const pendingHTMLStream = workUnitAsyncStorage.run(finalSSRPrerenderStore, renderToReadableStream, /*#__PURE__*/ _jsx(App, {
                                reactServerStream: rscForSSRStream,
                                preinitScripts: preinitScripts,
                                clientReferenceManifest: clientReferenceManifest,
                                ServerInsertedHTMLProvider: ServerInsertedHTMLProvider,
                ...
                if (SSRIsDynamic) {
                    ...
                    if (dynamicReason) {
                        ...
                        throw new DynamicServerError(`Route "${workStore.route}" couldn't be rendered statically it accessed data without explicitly caching it. See more info here: https://nextjs.org/docs/messages/dynamic-io`);
                    }
