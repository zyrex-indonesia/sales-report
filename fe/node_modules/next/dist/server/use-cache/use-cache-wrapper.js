...
function generateCacheEntry(workStore, outerWorkUnitStore, cacheScope, clientReferenceManifest, encodedArguments, fn) {
    // We need to run this inside a clean AsyncLocalStorage snapshot so that the cache
    // generation cannot read anything from the context we're currently executing which
    // might include request specific things like cookies() inside a React.cache().
    // Note: It is important that we await at least once before this because it lets us
    // pop out of any stack specific contexts as well - aka "Sync" Local Storage.
    return (0, _cleanasyncsnapshotexternal.runInCleanSnapshot)(generateCacheEntryWithRestoredWorkStore, workStore, outerWorkUnitStore, cacheScope, clientReferenceManifest, encodedArguments, fn);
}
function generateCacheEntryWithRestoredWorkStore(workStore, outerWorkUnitStore, cacheScope, clientReferenceManifest, encodedArguments, fn) {
    ...
    if (cacheScope) {
        return _cachescopeexternal.cacheScopeAsyncLocalStorage.run(cacheScope, ()=>_workasyncstorageexternal.workAsyncStorage.run(workStore, generateCacheEntryWithCacheContext, workStore, outerWorkUnitStore, clientReferenceManifest, encodedArguments, fn));
    }
...
async function generateCacheEntryImpl(workStore, outerWorkUnitStore, innerCacheStore, clientReferenceManifest, encodedArguments, fn) {
    ...
    if (workStore.isStaticGeneration) {
        ...
        timer = setTimeout(()=>{
            controller.abort(new Error('Filling a cache during prerender timed out like because request specific arguments such as ' + 'params, searchParams, cookies() or dynamic data was used inside the "use cache".'));
        }, 50000);
    }
...
function cache(kind, id, fn) {
    ...
    const cachedFn = {
        [name]: async function(...args) {
            ...
            const clientReferenceManifest = (0, _encryptionutils.getClientReferenceManifestSingleton)();
            ...
            if (cacheScope) {
                const cachedEntry = cacheScope.cache.get(serializedCacheKey);
            ...
            if (stream === undefined) {
                ...
                if (workUnitStore !== undefined && workUnitStore.type === 'prerender' && entry !== undefined && (entry.revalidate === 0 || entry.expire < DYNAMIC_EXPIRE)) {
                    ...
                    // Note: It is important that we await at least once before this because it lets us
                    // pop out of any stack specific contexts as well - aka "Sync" Local Storage.
                    const [newStream, pendingCacheEntry] = await generateCacheEntry(workStore, workUnitStore, cacheScope, clientReferenceManifest, encodedArguments, fn);
                    let savedCacheEntry;
                    if (cacheScope) {
                    ...
                    if (currentTime > entry.timestamp + entry.revalidate * 1000) {
                        ...
                        const [ignoredStream, pendingCacheEntry] = await generateCacheEntry(workStore, undefined, cacheScope, clientReferenceManifest, encodedArguments, fn);
                        const promise = cacheHandler.set(serializedCacheKey, pendingCacheEntry);
                        if (!workStore.pendingRevalidateWrites) {
                            workStore.pendingRevalidateWrites = [];
                        }
                        workStore.pendingRevalidateWrites.push(promise);
