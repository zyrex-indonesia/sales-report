...
async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, experimentalBuildMode, traceUploadUrl) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            await (0, _builddiagnostics.recordFrameworkVersion)("15.0.1");
            ...
            const { customAppGetInitialProps, namedExports, isNextImageImported, hasSsrAmpPages, hasNonStaticErrorPage } = await staticCheckSpan.traceAsyncFn(async ()=>{
                ...
                await Promise.all(Object.entries(pageKeys).reduce((acc, [key, files])=>{
                    ...
                    return checkPageSpan.traceAsyncFn(async ()=>{
                        ...
                        pageInfos.set(page, {
                            ...
                            unsupportedSegmentConfigs: staticInfo && 'unsupportedSegmentConfigs' in staticInfo ? staticInfo.unsupportedSegmentConfigs : undefined
                        });
                    });
                }));
                ...
                if (config.experimental.dynamicIO) {
                    const pagesWithSegmentConfigs = [];
                    pageInfos.forEach((pageInfo, page)=>{
                        if (pageInfo.unsupportedSegmentConfigs && pageInfo.unsupportedSegmentConfigs.length > 0) {
                    ...
                    if (pagesWithSegmentConfigs.length > 0) {
                        _log.error(`The following pages used segment configs which are not supported with "experimental.dynamicIO" and must be removed to build your application:\n${pagesWithSegmentConfigs.join('\n')}\n`);
                        process.exit(1);
                    }
                }
